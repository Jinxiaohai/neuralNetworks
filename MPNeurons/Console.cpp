...
void Console::destory()
{
  ...
  if(delno==0)
    ...
    {	//3 situations when you want to delete a element in a numbergroup
      if(NumofNeurs==1)
        {
...
void Console::status()
{	//useless format control without GUI
  cout<<"No."<<setw(7)<<"Type"<<setw(19)<<"Threshold"<<setw(15)<<"Status\t"
      <<setw(19)<<"Connection\n";
...
void Console::connect()
{
  ...
  if(con1st<=0||con1st>NumofNeurs)
    {	//in actually, con1st==0 is not necessary a exception, same as below
      cout<<"Error Flag 1.\n";	//EH
      return;
    }
  if(NeuronsList[con1st-1]->getType()=="Output Neuron")
    {	//an output neuron cannot connect to other
  ...
  if(NeuronsList[con2nd-1]->getType()=="Input Neuron")
    {	//an input neuron cannot be connected to other
...
{
  if(histemp!="")
    {	//Advance: redo temp number can set. Can also use deque to apply
